...
export default function usersReducer(state = initialState, action) {
  if (!(state instanceof UsersState)) return revive(state);

  switch (action.type) {
    case authActions.LOGIN_SUCCESS: {
      const { firstName, email, userId, lastName } = action.payload;
      const displayName = `${firstName} ${lastName}`;

      const user = new User({ authenticated: true, displayName, email, userId });
